...
USETFAverage <- getStocksAverageTimeSeries(paste(dataPath, "USTechETF/",sep=""),"USETFAverage") %>%
  mutate(year=year(date)+month(date)*0.01) %>%
  group_by(year) %>%
  summarise(price=mean(price))
ChinaTechInUSYearly <- ChinaTechInUS %>%
  mutate(year=year(date)+month(date)*0.01) %>%
  group_by(year) %>%
  summarise(price=mean(price))
USvsChina <- data.frame(us=USETFAverage$price, china=ChinaTechInUSYearly$price)
ggplot(USvsChina, aes(x=us, y=china)) + geom_jitter() + geom_smooth(method="lm")
```
Not very strong correlation, calculated roughly 0.3
...
plot and calculate correlation:
```{r}
ChinaTechDomesticYearly <- ChinaTechRMB %>%
  mutate(year=year(date)+month(date)*0.01) %>%
  group_by(year) %>%
  summarise(price=mean(price))
ForeignVSDomestic <- data.frame(foreign=ChinaTechInUSYearly$price, domestic=ChinaTechDomesticYearly$price)
ggplot(ForeignVSDomestic, aes(x=domestic, y=foreign)) + geom_jitter() + geom_smooth(method="lm")
```
